...
describe('CreativeItem', () => {
  it("should render with title, contributors and formats'", () => {
    ...
    expect(container.querySelector(`[data-testid="creative-item-${defaultProps.creative.id}"]`)).toBeVisible();
    ...
    expect(contributors).toHaveLength(defaultProps.creative.contributors.length);
  ...
  it('should render with bold title if selected', () => {
    const { container } = render(<CreativeItem {...defaultProps} isSelected={true} />);
